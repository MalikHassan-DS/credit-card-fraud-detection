...
@st.cache_resource
def load_model():
    model = joblib.load(MODEL_DIR / "fraud_detection_random_forest.pkl")
...
st.title("💳 Credit Card Fraud Detection System")
st.markdown("This application uses a **Random Forest Machine Learning model** to detect potentially fraudulent credit card transactions.")
st.divider()

if df is None:
    st.error("❌ test_transactions.csv was not found. Please place it in the same folder as app.py.")
    st.stop()
...
with col3:
    st.metric("Test Transactions", f"{len(df):,}")
...
st.info("Load a real test transaction stored in the project test dataset. You do not need to enter V1–V28 manually.")
...
with col1:
    if st.button("🟢 Load Normal Transaction", use_container_width=True):
        rows = df[df["Class"] == 0]
        idx = rows.index[0]
        st.session_state.selected_transaction = df.loc[idx]
...
if selected_transaction is None:
    st.info("Choose **Load Normal Transaction** or **Load Fraud Transaction** to fill the transaction values automatically.")
    st.stop()
...
col1, col2 = st.columns(2)
with col1:
    input_data["Time"] = st.number_input("Time", value=float(selected_transaction["Time"]), format="%.6f", key="time_input")
with col2:
    input_data["Amount"] = st.number_input("Transaction Amount", min_value=0.0, value=float(selected_transaction["Amount"]), format="%.6f", key="amount_input")

st.subheader("Transaction Features V1 - V14")
cols = st.columns(3)
for i, feature in enumerate(FEATURES[1:15]):
    with cols[i % 3]:
        input_data[feature] = st.number_input(feature, value=float(selected_transaction[feature]), format="%.6f", key=f"{feature}_input")
...
with st.expander("📄 View Complete Transaction Data"):
    st.dataframe(pd.DataFrame([selected_transaction[FEATURES].to_dict()]), use_container_width=True)

st.divider()
if st.button("🔍 Analyze Transaction", use_container_width=True):
    transaction = pd.DataFrame([input_data], columns=FEATURES)
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.error("Actual Label: 🔴 FRAUD (1)") if actual_label == 1 else st.success("Actual Label: 🟢 NORMAL (0)")
    with col2:
        st.error("Predicted Label: 🔴 FRAUD (1)") if prediction == 1 else st.success("Predicted Label: 🟢 NORMAL (0)")

    if actual_label == prediction:
        st.success("✅ Model Prediction is CORRECT")
    else:
        st.warning("⚠️ Model Prediction is INCORRECT")
